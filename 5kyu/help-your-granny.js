...
function tour(friends, fTowns, distTable) {
  const visitRoute = friends
    .filter((friend) => fTowns.some((ft) => ft.includes(friend)))
    .map((vr) => {
      const [friend, dist] = fTowns.find((ft) => ft[0] === vr);
      const index = distTable.findIndex((d) => d === dist);
      const distance = distTable[index + 1];
      return distance;
    });
  let totalDistance = visitRoute[0];
  for (let i = 0; i < visitRoute.length; i++) {
     if (i === visitRoute.length - 1) {
      totalDistance += visitRoute[i];
    } else {
      const a = visitRoute[i] ** 2;
      const c = visitRoute[i + 1] ** 2;
      totalDistance += Math.sqrt(c - a);
    }
  }
