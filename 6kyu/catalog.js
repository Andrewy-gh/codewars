...
function catalog(s, article) {
  const lines = s.split('\n').filter(e => e)
  const search = lines.filter(e => e.includes(article))
  if (!search.length) return 'Nothing'
  const result = search.map((e,i) => {
    const name = search[i].split('name>')[1].slice(0, -2)
    const prx = search[i].split('prx>')[1].slice(0, -2)
    const qty = search[i].split('qty>')[1].slice(0, -2)
